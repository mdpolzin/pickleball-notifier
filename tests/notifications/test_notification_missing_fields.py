...
def test_load_subgroup_id_missing_field_raises_runtime_error(tmp_path, monkeypatch) -> None:
...
def test_load_access_token_missing_field_raises_runtime_error(tmp_path, monkeypatch) -> None:
    monkeypatch.chdir(tmp_path)
    (tmp_path / "config.json").write_text(
        json.dumps(
            {"groupme": {"subgroup_id": "sub-1"}, "player": {"slug": "jane-doe"}}
        ),
        encoding="utf-8",
    )
    handler = NotificationHandler.__new__(NotificationHandler)
