...
class ConfigManager:
    ...
    def update_matches(self, match_urls: List[str]) -> Dict[str, int]:
        ...
        for uuid in current_uuids:
            ...
            if uuid in self.matches:
                ...
                self.matches[uuid].consecutive_stale = 0
                existing_matches += 1
            else:
                # Add new match
    ...
    def get_pending_notifications(self) -> List[MatchInfo]:
        """Get matches with court assignments that have not been notified yet and are not completed."""
        return [
    ...
    def remove_stale_matches(self, current_uuids: Set[str]) -> int:
        ...
        if not matches_to_remove:
            print(
                "Tracking "
                f"{len(stale_uuids)} potentially stale matches "
                f"(consecutive count: {min(self.matches[uuid].consecutive_stale for uuid in stale_uuids)}-"
                f"{max(self.matches[uuid].consecutive_stale for uuid in stale_uuids)})"
            )
            return 0
        ...
        for uuid in matches_to_remove:
            match = self.matches[uuid]
            print(
                "  - Removing stale match: "
                f"{uuid} (status: {match.status}, court: {match.court_title or 'none'}, "
                f"consecutive_stale: {match.consecutive_stale})"
            )
